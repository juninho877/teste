...
// Atores
$actorYPosition = $imageHeight - 330;
$actorWidth = 65;
$actorHeight = 97;
$actorXPosition = 25;

foreach ($atores as $ator) {
    $actorImageUrl = "https://image.tmdb.org/t/p/w45" . $ator['profile_path'];
...
function applyRoundedCorners($image, $radius = 20) {
    ...
    // Cria uma imagem com canal alpha (transparência)
    ...
    // Cria a máscara com cantos arredondados
    for ($x = 0; $x < $width; $x++) {
        for ($y = 0; $y < $height; $y++) {
            $alpha = 127;
            ...
            // Cálculo dos cantos
            $inCorner = false;
            if ($x < $radius && $y < $radius) { // canto superior esquerdo
                $inCorner = (pow($x - $radius, 2) + pow($y - $radius, 2)) <= pow($radius, 2);
            } elseif ($x >= $width - $radius && $y < $radius) { // canto superior direito
                $inCorner = (pow($x - ($width - $radius - 1), 2) + pow($y - $radius, 2)) <= pow($radius, 2);
            } elseif ($x < $radius && $y >= $height - $radius) { // canto inferior esquerdo
                $inCorner = (pow($x - $radius, 2) + pow($y - ($height - $radius - 1), 2)) <= pow($radius, 2);
            } elseif ($x >= $width - $radius && $y >= $height - $radius) { // canto inferior direito
...
// Agora cole a imagem com bordas arredondadas no destino
